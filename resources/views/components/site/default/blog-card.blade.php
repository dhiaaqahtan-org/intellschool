            ...
            <div class="flex justify-between">
                ...
                @if ($blog->category)
                    <p class="text-site-primary text-sm font-medium">
                        <a
                            href="{{ route('site.page.blog-list-category', ['slug' => $slug, 'category' => $blog->category->slug]) }}">
                            <x-ui.badge color="custom" :color-value="$blog->category->color">{{ $blog->category->name }}</x-ui.badge>
                        </a>
                    </p>
                @endif
            </div>
            <a href="{{ '/pages/b/' . $slug . '/' . $blog->slug }}" class="mt-2 block">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
                @foreach ($blog->tags as $tag)
                    <a class="truncate rounded-lg bg-gray-200 px-2 py-1 text-sm text-gray-800 dark:bg-gray-500 dark:text-gray-300"
                        href="{{ route('site.page.blog-list-tag', ['slug' => $slug, 'tag' => strtolower($tag->name)]) }}">#{{ $tag->name }}</a>
                @endforeach
            </div>
        @endif
    </div>
